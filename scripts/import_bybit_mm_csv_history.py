#!/usr/bin/env python3
"""Import Bybit MM notebook trade CSVs into the normalized PostgreSQL table."""
...
SUPPORTED_STRATEGY = "bybit_mm_alpha"
EXPECTED_SCHEMA = "bybit_mm_alpha"
EXPECTED_COLUMNS = (
    "symbol",
    "id",
    "orderId",
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Import Bybit MM notebook CSVs into normalized trades"
    )
...
def load_rows(data_dir, start_ms, end_ms):
        ...
        with open(path, newline="") as source:
            ...
            for line_number, raw in enumerate(reader, 2):
                ...
                if end_ms is not None and ts >= end_ms:
                    at_or_after_end += 1
                    continue
                raw_key = (row["market"], row["symbol"], row["trade_id"])
                if raw_key in colliding_keys:
                    row = dict(row)
                    row["trade_id"] = f"{row['trade_id']}:{row['order_id']}"
                    ...
                    if previous is not None:
                        ...
                        if previous != row:
                            raise RuntimeError(
                                f"conflicting self-trade duplicate: {key!r}"
                            )
                    rows[key] = row
                    continue

                previous = rows.get(raw_key)
                if previous is None:
                    rows[raw_key] = row
                    continue
                if previous == row:
                ...
                rows.pop(raw_key)
                colliding_keys.add(raw_key)
                self_trade_collisions += 1
                for item in (previous, row):
                    item = dict(item)
                    item["trade_id"] = f"{item['trade_id']}:{item['order_id']}"
                    key = (item["market"], item["symbol"], item["trade_id"])
                    duplicate = rows.get(key)
                    if duplicate is not None and duplicate != item:
                        raise RuntimeError(f"conflicting self-trade row: {key!r}")
                    rows[key] = item
    ordered = sorted(
        rows.values(),
    ...
    print(
        ...
        f"duplicates={duplicates}, self_trade_collisions={self_trade_collisions}, "
        f"before_start={before_start}, at_or_after_end={at_or_after_end}",
        flush=True,
    )
    return ordered
...
def copy_to_postgres(database, schema, rows):
    ...
    stream.close()
    if process.wait() != 0:
        raise RuntimeError("psql Bybit MM CSV import failed")
...
def main():
    ...
    if rows:
        print(
            f"Bybit MM CSV import complete: strategy={args.strategy}, "
            f"rows={len(rows)}, start_ms={rows[0]['event_time_ms']}, "
            f"end_ms={rows[-1]['event_time_ms']}",
            flush=True,
        )
    else:
        print(f"Bybit MM CSV import complete: strategy={args.strategy}, rows=0")
