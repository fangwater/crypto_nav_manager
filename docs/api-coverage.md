...
正式运行时由 `BybitClient` 直接鉴权请求交易所，按 7 天窗口切分并使用 `nextPageCursor` 分页。
查询只支持 linear/inverse 衍生品，并明确排除 `CreateByAdl_PassThrough`。现有 `scripts/import_bybit_*_history.py` 仅用于一次性历史回填，不是运行时数据源。
...
- Bybit 的动态成交、资金费、借贷和手续费率尚未迁入 Rust 客户端；现有 Python 脚本只是历史回填工具。账户强平查询已由 `BybitClient` 动态拉取。
